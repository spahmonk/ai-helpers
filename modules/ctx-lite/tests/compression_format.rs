...
fn test_minifier_javascript_signature() {
    let input =
        "async function fetchData ( url : string , options : Config ) : Promise < Response >";
...
fn test_minifier_complex_rust_generics() {
    ...
    assert!(
        ...
        "Minified should have fewer spaces"
    );
...
fn test_compressed_format_enum_full() {
    ...
    assert_eq!(
        result, content,
        "Full format should return unchanged content"
    );
}
...
fn test_compressed_format_enum_minified() {
    ...
    assert!(
        ...
        "Minified should be equal or smaller"
    );
}
...
fn test_minifier_typescript_generics() {
    ...
    assert!(
        spaces < input_spaces,
        "TypeScript signature should be minified"
    );
}
